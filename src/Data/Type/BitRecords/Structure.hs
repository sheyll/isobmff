...
-- | Structure definition for sequences of bits.
--
-- This module provides the core data types and functions for
-- mapping finite bit sequences from/to conventional (Haskell) data types.
--
...
--
-- Complete bit sequence may be constructed or destructed from and to Haskell types.
...
-- | Indicate if a 'Structure' has a statically known size.
...
-- | The number of bits that a structure with a predetermined fixed size requires.
type family GetStructureSize2 t :: Nat

-- | Empty Structure
data EmptyStructure2
type instance GetStructureSize2 EmptyStructure2 = 0
...
-- | A constant, fixed length sequence of bits, generated by a type level 'TypeLiterals'.
