...
spec = do
  describe "StaticExpandable" $ do
    describe "ExpandableSizeLastChunk"
      $ it "renders both 2 and 130 as 00000010 "
      $ do
          let
            actual130 = bitBuffer64Printer
              (Proxy :: Proxy ((ExpandableSizeLastChunk 130)))
            actual2 =
              bitBuffer64Printer (Proxy :: Proxy (ExpandableSizeLastChunk 2))
          actual130 `shouldBe` actual2
          actual2 `shouldBe` "<< 02 >>"
    describe "ExpandableSize"
      $ it "creates a stdandard conform size representation for the size 130"
      $ let actualStr =
              bitBuffer64Printer (Proxy :: Proxy (ExpandableSize 130))
    ...
    it "writes the size 128 as [ 0b10000001, 0b00000000 ] "
      $ let actual =
              B.unpack $ toLazyByteString (boxBuilder (Expandable (ETC 128)))
            expected = [129, 0]
        in  actual `shouldBe` expected
    it
        "writes the size (2 ^ 21) as [ 0b10000001, 0b10000000, 0b10000000, 0b00000000 ] "
      $ let
          actual =
            B.unpack $ toLazyByteString
              (boxBuilder (Expandable (ETC (2 ^ (21 :: Int)))))
          expected = [129, 128, 128, 0]
        in
          actual `shouldBe` expected
    it "writes the size (2 ^ 21 - 1) as [ 0b11111111, 0b11111111, 0b01111111 ] "
      $ let actual = B.unpack $ toLazyByteString
              (boxBuilder (Expandable (ETC (2 ^ (21 :: Int) - 1))))
            expected = [255, 255, 127]
        in  actual `shouldBe` expected
    it "writes size according to the spec" $ property $ \etc@(ETC s) ->
      let expectedBoxSize = BoxSize $ s + if s < 2 ^ (7 :: Int)
            then 1
            else if s < 2 ^ (14 :: Int)
              then 2
              else if s < 2 ^ (21 :: Int) then 3 else 4
          actualBoxSize = boxSize (Expandable etc)
      in  actualBoxSize `shouldBe` expectedBoxSize
